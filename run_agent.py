...
class AIAgent:
    ...
    # --- Custom patch (Deep/Nyx fork) ---------------------------------------
    def _read_prep_file_context(self, opener_date: str) -> Optional[str]:
        """Read NyxBrain's pre-checkin prep file for the given opener_date and
        return the non-opener sections (Context for Hermes / Trend signals /
        Topics / Tone) if the file is fresh.

        NyxBrain (the smart-brain Claude persona at ``~/.hermes/NyxBrain/``)
        runs at 21:15 IST every night and writes a prep file with five
        sections — the ``## Opener`` line that Hermes posts verbatim at 21:30,
        plus four supporting sections of situational awareness for the
        reply-turn agent: Context for Hermes, Trend signals (from rolling
        7 days), Topics to weave in, Tone. The reply-turn agent (this
        addendum's caller) uses those four sections to inform conversation
        behavior — they are NOT to be repeated back to Deep verbatim nor used
        as conversation prompts; they're background awareness so Hermes can
        weight tone, follow up on the right threads, and leave the right
        things alone.

        Returns None for any of: missing file, stale ``prep_for`` field
        (doesn't match opener_date), read failure, malformed (no extractable
        non-opener content). The reply-turn agent then runs without prep
        context — same as before this patch — which is the safe degraded
        state.

        Path is overrideable via ``DAILY_CHECKIN_PREP_DIR`` env var for tests.
        """
        import os
        import re as _re
        from pathlib import Path as _Path

        prep_dir = _Path(os.environ.get(
            "DAILY_CHECKIN_PREP_DIR",
            os.path.expanduser("~/.hermes/state"),
        ))
        prep_file = prep_dir / f"checkin_prep_{opener_date}.md"
        ...
        try:
            ...
            return None
        # Freshness check: prep_for must match the opener_date — guards
        # against stale prep files lingering from a prior day's checkin.
        if f"prep_for: {opener_date}" not in content:
            return None
        # Strip frontmatter (between two `---` lines at the start).
        if content.startswith("---"):
            end = content.find("\n---\n", 4)
            if end != -1:
                content = content[end + 5:]
        # Strip the `# Daily Checkin Prep — <date>` H1 title (metadata, not
        # content — the addendum already announces "Daily Checkin in progress"
        # in its own H1, so this would just be redundant).
        content = _re.sub(
            r"^# Daily Checkin Prep[^\n]*\n",
            "",
            content,
            count=1,
            flags=_re.MULTILINE,
        )
        # Drop the `## Opener` section — that's already been posted to Discord
        # by the 21:30 cron; the reply-turn agent doesn't need it again. Match
        # from the `## Opener` heading up to (not including) the next `## `
        # heading at column 0, or end of file.
        content = _re.sub(
            r"^## Opener\s*\n.*?(?=^## |\Z)",
            "",
            content,
    ...
    def _build_daily_checkin_addendum(self) -> Optional[str]:
        """If a Daily Checkin is open and this session is bound to its
        delivery target, return a system-prompt addendum that mandates the
        `deep-second-brain` skill load + the touch/close protocol.

        Mechanism: reads ``~/.hermes/state/evening_checkin_active.json``. If
        ``state == "open"`` and the value of ``thread_id`` (post-2026-05-12
        migration this is the Discord ``#checkin`` channel id) appears in
        this session's ``_gateway_session_key``, inject the addendum.
        Otherwise return None.

        Phase 2 of the Daily Checkin two-brain redesign (2026-05-15) also
        reads NyxBrain's pre-checkin prep file at
        ``~/.hermes/state/checkin_prep_<opener_date>.md`` and injects its
        non-opener sections (Context / Trend signals / Topics / Tone) into
        the addendum so the reply-turn agent's responses are informed by the
        same synthesis NyxBrain used to pick tonight's opener. If the prep
        file is missing/stale/malformed, the addendum still fires but without
        the prep context — same behavior as pre-Phase-2.

        Returning None for any read failure or state mismatch is the safe
        default — the agent falls back to its prior generic behavior, which
        was the failure mode this patch is fixing but at least doesn't make
        things worse.
        """
        import json as _json
        from pathlib import Path as _Path

        state_path = _Path(os.environ.get(
            "DAILY_CHECKIN_STATE_FILE",
            os.path.expanduser("~/.hermes/state/evening_checkin_active.json"),
        ))
        ...
        if state.get("state") != "open":
            return None
        target_thread = str(state.get("thread_id") or "").strip()
        if not target_thread:
            return None
        # Confirm this session is bound to the state's delivery target.
        # Match against the GATEWAY SESSION KEY, not self.session_id:
        # self.session_id is the timestamp-based DB id (e.g.
        # 20260513_213245_2b72fb41) and never contains the channel id.
        # self._gateway_session_key is the stable per-chat routing key:
        #   Discord:   agent:main:discord:group:<channel_id>:<user_id>
        #   Telegram:  agent:main:telegram:dm:<chat_id>:<thread_id>
        # The thread_id from state appears as a path segment there.
        # Substring match is sufficient (no overlap risk: ids are long
        # opaque integers, not collidable). Cron sessions have no gateway
        # session key -> "" -> no match -> addendum correctly skipped.
        session_key = (self._gateway_session_key or "")
        if target_thread not in session_key:
            return None
        opener_date = state.get("opener_date") or "?"
        script = "/Users/nyx/.hermes/scripts/evening_checkin_opener.py"
        daily_path = (
            f"/Users/nyx/Documents/Hermes Vault/Daily/{opener_date}.md"
        )
        # turn_count was just incremented by the checkin_touch gateway hook
        # (~/.hermes/hooks/checkin_touch/) on agent:start, so what we read
        # here is the post-touch value — i.e. THIS reply's turn number.
        # Added 2026-05-18 alongside the hook to eliminate the LLM-side
        # touch call; see HANDOFF_NEXT_DAILY_CHECKIN.md `## 2026-05-17`
        # entry for the failure mode this replaces.
        current_turn = int(state.get("turn_count") or 0)

        # Phase 2: pull NyxBrain's prep context (Context / Trend signals /
        # Topics / Tone) if the prep file is fresh. Empty string if not —
        # the addendum still fires with full protocol; only the situational-
        # awareness layer is degraded.
        prep_context = self._read_prep_file_context(opener_date)
        if prep_context:
            prep_block = (
                f"## NyxBrain prep for tonight\n\n"
                f"NyxBrain (the smart-brain Claude persona) ran at 21:15 "
                f"and synthesized the day's context ahead of this conversation "
                f"so you wouldn't have to. The opener Deep just saw was "
                f"picked from this same prep. The sections below are "
                f"**situational awareness** for YOU — informing tone, what "
                f"to follow up on, what to leave alone — they are NOT "
                f"conversation prompts to repeat back at Deep, NOT a "
                f"checklist to work through, NOT to be cited explicitly. "
                f"Use them to shape your responses; don't surface them.\n\n"
                f"{prep_context}\n\n"
                f"---\n\n"
            )
        else:
        ...
        return (
            f"# Daily Checkin in progress — opener_date={opener_date}\n\n"
            + prep_block
            + f"`~/.hermes/state/evening_checkin_active.json` shows "
            f"`state: open` for opener_date `{opener_date}`, bound to this "
            f"channel (thread_id={target_thread}). This conversation IS Deep's "
            f"Daily Checkin flow — not a generic chat.\n\n"
            f"**LISTEN, DON'T OPERATE.** The checkin is a listening space. "
            f"When Deep mentions a problem, a bug, a broken task, or "
            f"something that went wrong (\"screenshots were failing\", "
            f"\"the deploy broke\"), he is *talking about his day* — he is "
            f"NOT asking you to fix it. DO NOT use tools to investigate, "
            f"diagnose, debug, or act on anything he brings up. Hear it, "
            f"acknowledge it as part of his day, let him keep talking. The "
            f"ONLY tools you touch during a checkin are: `skill_view`, "
            f"`session_search` (for conversation context), and at close the "
            f"file write + `close` call. If something genuinely needs an "
            f"action item it is routed AT CLOSE (Reminders / pending "
            f"decisions) — never acted on mid-conversation.\n\n"
            f"**This reply is turn N = {current_turn}.** The gateway's "
            f"`checkin_touch` hook (`~/.hermes/hooks/checkin_touch/`) "
            f"already incremented `turn_count` and refreshed "
            f"`last_activity_at` in the state file when Deep's message "
            f"arrived — that's where this N comes from. Use N for the "
            f"TURN-1 MARKER decision below.\n\n"
            f"**Mandatory protocol** (per the `deep-second-brain` skill, "
            f"`## Daily Checkin` section):\n\n"
            f"1. **Load the skill before replying** (every fresh session in this "
            f"channel during an open checkin): call "
            f"`skill_view(name=\"deep-second-brain\")` and read the "
            f"`## Daily Checkin` section. The skill is authoritative on "
            f"Reflection format, output discipline, and the pre-save "
            f"verification checklist.\n"
            f"2. **DO NOT call `{script} touch`** — the gateway-side "
            f"`checkin_touch` hook does it deterministically when Deep's "
            f"message arrives (added 2026-05-18 to replace the previous "
            f"LLM-side instruction, which Hermes inconsistently followed). "
            f"A manual touch from your side would double-count `turn_count` "
            f"and corrupt the idle-sweep / carry-over signal.\n"
            f"3. **TURN-1 MARKER**: If `N == 1` (your first reply in this "
            f"checkin — read N from the bold line above), prepend exactly "
            f"this line plus a blank line to your reply: "
            f"`🟢 _Checkin mode · {opener_date} · turn 1_`. Deep's visible "
            f"confirmation that the mandate loaded. If `N > 1`, DO NOT "
            f"prepend it — the marker fires exactly once per checkin.\n"
            f"4. **CLOSE at conversation end** — when Deep signals end (says "
            f"'bye', 'done', 'skip', 'not now', or the exchange winds down "
            f"with no follow-up hook). First decide which branch this is, "
            f"then run it. EITHER WAY a visible 🔻 message is posted — Deep "
            f"wants a close notification on EVERY checkin. The `close` "
            f"subcommand's `--reason` MUST be exactly `completed` or "
            f"`skipped` — do not invent other values.\n"
            f"   **4A — COMPLETED** (Deep shared real substance about his "
            f"day, even briefly):\n"
            f"   (a) Compose the Reflection per the skill's required "
            f"structure: first-person ghost-write, multi-paragraph "
            f"narrative, wikilinks on proper nouns only, `**What mattered:**` "
            f"/ `**Unresolved:**` / `**Tomorrow's anchor:**` lines, affective "
            f"`_Tone:_` line (no productivity vocabulary). Hold it in "
            f"memory.\n"
            f"   (b) Write it under a fresh `## Reflection` heading in "
            f"`{daily_path}` (replace the `_Pending_` placeholder if "
            f"present). Use the file_write or edit tool.\n"
            f"   (c) Call `{script} close --reason completed --turns <N>`.\n"
            f"   (d) Post a SINGLE message: `🔻 Checkin closed for "
            f"{opener_date}`, blank line, the full Reflection content "
            f"verbatim (Deep sees it in chat AND the vault), blank line, "
            f"`Saved to Daily/{opener_date}.md. Want any edits?`\n"
            f"   **4B — SKIPPED** (Deep declined, said skip/not now, or the "
            f"exchange had no day-content — e.g. he was testing the "
            f"mechanism or just poking):\n"
            f"   (a) Write a skip marker under a fresh `## Reflection` "
            f"heading in `{daily_path}` (replace `_Pending_` if present): a "
            f"line `_Skipped — <one short honest line on what happened>._`, "
            f"blank line, then verbatim so the parser stays valid: "
            f"`**What mattered:** (none — checkin skipped)` / "
            f"`**Unresolved:** (none captured)` / "
            f"`**Tomorrow's anchor:** (none — checkin skipped)`, blank line, "
            f"`_Tone: (unknown — checkin skipped)_`.\n"
            f"   (b) Call `{script} close --reason skipped --turns <N>`.\n"
            f"   (c) Post a SINGLE message: `🔻 Checkin skipped for "
            f"{opener_date}`, blank line, one or two plain sentences on what "
            f"actually happened (so Deep has the record), blank line, "
            f"`Marked skipped in Daily/{opener_date}.md.`\n"
            f"   In BOTH branches: DO NOT prepend the turn-1 🟢 marker on "
            f"the close turn — the 🔻 replaces it.\n"
            f"5. **POST-CLOSE MODE**: If on a future turn the state file "
            f"shows `state == \"closed\"` (the checkin already ended this "
            f"cycle but you're still being asked this addendum because it's "
            f"cached in your system prompt), DO NOT re-run touch or close — "
            f"the protocol is done. Treat the turn as normal chat. If Deep "
            f"asks for edits to today's Reflection, edit the file at "
            f"`{daily_path}` directly and confirm tersely. If Deep starts a "
            f"new topic, just chat normally.\n\n"
            f"This addendum is injected by the state file, not by the cron "
            f"prompt — it persists across gateway-session restarts as long "
            f"as the checkin is open. The Layer-2 idle sweep (cron, every "
            f"10 min) will trigger this same close protocol from an external "
            f"process if Deep goes silent for >30 min after engaging — same "
            f"output, same quality."
        )
    # --- End custom patch ---------------------------------------------------
